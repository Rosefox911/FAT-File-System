...
//Global variables
vcb myvcb; // global variable for VCB
dirent mydirent[100]; // Creating an array of structures

//Helper function
void write_dirent (int i) {
  char tmp[BLOCKSIZE];
  memset(tmp, 0, BLOCKSIZE);
  memcpy(tmp, &mydirent[i], sizeof(dirent));
  dwrite(i+1,tmp);
  return;
}
...
static void* vfs_mount(struct fuse_conn_info *conn) {
  ...
  for (int i=0; i<100; i++) {
	  ...
	  memcpy(&mydirent[i], detmp, sizeof(dirent));//Copying detmp to mydirent[i]
  }
...
static void vfs_unmount (void *private_data) {
  ...
  //write all mydirent's to disk
  for(int i = 0; i < 100; i++) {
    char buf[BLOCKSIZE];
    memset( buf, 0, BLOCKSIZE);
    memcpy( buf, &mydirent[i], sizeof(mydirent[i]));
    dwrite( 1 + i, buf);
  }
 
  // write the fat table to disk too 
  stopfat();
...
static int vfs_getattr(const char *path, struct stat *stbuf) {
  ...
  // because its entry does not exist in the mydirent array
  if (strcmp(path, "/") == 0) {
  ...
  for(int i = 0; i < 100; i++) {
    if ((strcmp(mydirent[i].name, path) == 0) && (mydirent[i].valid == 1)) {
      fprintf( stderr, "In getattr: found the file %s in dirents\n", path);
	  stbuf->st_uid =  mydirent[i].user;
	  stbuf->st_gid = mydirent[i].group;
	  //Crates ointers to structures so I can use them to get time
	  struct tm * accesstm;
	  struct tm * modifytm;
	  struct tm * createtm;
	  accesstm = localtime(&((mydirent[i].access_time).tv_sec));
	  modifytm = localtime(&((mydirent[i].modify_time).tv_sec));
	  createtm = localtime(&((mydirent[i].create_time).tv_sec));
	  
	  stbuf->st_atime = mktime(accesstm);
	  stbuf->st_mtime = mktime(modifytm);
	  stbuf->st_ctime = mktime(createtm);

      stbuf->st_mode = mydirent[i].mode;
	  stbuf->st_size = mydirent[i].size;
    if( mydirent[i].size == 0) 
      stbuf->st_blocks = 1;//a special case cause we allocate a block just when a file is creted
    else
  	  stbuf->st_blocks  = ((mydirent[i].size -1) /BLOCKSIZE)  + 1;
  	  return 0;
    }
  } //end of for loop
...
static int vfs_mkdir(const char *path, mode_t mode) {
  fprintf( stderr, "In vfs_mkdir: creating directory %s\n", path);
  char filepath[128];
  strcpy(filepath, path);
  // remove any trailing slash;strlen>1 cause we wont remove / for rootdir
  if( strlen(filepath) > 1 && path[strlen(filepath)-1] == '/') 
     filepath[strlen(path)-1] = '\0';
  
  int validdir = 0;
  // get the directory in which this file is being created using getdir
  char *dir = get_dir(filepath);
  // if this directory is not being created right in root dir, make sure
  // that its parent directory exists
  if(strcmp(dir, "")) { // dir is "" when file is being created in rootdir
    for(int i = 0; i < 100; i++) {
      if ( !strcmp(mydirent[i].name, dir) && (mydirent[i].valid == 1)
          && (mydirent[i].mode & S_IFDIR)) {
	      validdir = 1;	
      }
    }
    if(validdir == 0)
  ...
  for(int i = 0; i < 100; i++) {
    if (strcmp(mydirent[i].name, filepath)==0) {
      return -EEXIST;
    }
  }
  int i;
  for( i = 0; i < 100; i++) {//Loops through all dirents till a dirent is found
    if (mydirent[i].valid == 0) {
      fprintf(stderr, "In vfs_mkdir: Found an empty dirent in %i, writing to it\n",i); //Tells us which block it is in
      mydirent[i].valid = 1;
      // for a directory, we do not allocate any fatent at all, and 
      // nor is one used even later on
      
		  //Fill in getuid, getgid mode and path
      mydirent[i].user = getuid();
      mydirent[i].group = getgid();
      mydirent[i].mode = mode | S_IFDIR;
      strcpy(mydirent[i].name, filepath);
      clock_gettime(CLOCK_REALTIME, &mydirent[i].access_time);
      clock_gettime(CLOCK_REALTIME, &mydirent[i].modify_time);
      clock_gettime(CLOCK_REALTIME, &mydirent[i].create_time);
      write_dirent(i);
      fprintf( stderr, "In vfs_mkdir, created the directory\n");
  ...
  if(i == 100) { 
    fprintf(stderr, "Error: can't create directory cause dirent is full\n");
...
static int vfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi) {
  ...
  filler( buf, "..", 0, 0);
  for(i = 0; i < 100; i++) {
    if( mydirent[i].valid == 1) {
      fprintf(stderr, "going to check isfilein for file = %s dir = %s\n", mydirent[i].name, dirpath);
      if (isfilein( mydirent[i].name, dirpath) ) {
         fprintf(stderr, "yes, file is in the given directory\n");
         struct stat stats;
         stats.st_mode = mydirent[i].mode;        
         stats.st_uid = mydirent[i].user;
         stats.st_gid = mydirent[i].group;
         if( !strcmp(dirpath, "/"))
           filler( buf, mydirent[i].name + 1, &stats,0);   
         else
           filler( buf, mydirent[i].name + strlen(dirpath) + 1, &stats,0);
      }
    }  
  }
...
static int vfs_create(const char *path, mode_t mode, struct fuse_file_info *fi) {
  ...
  if(strcmp( dir, "")) {
    for(i = 0; i < 100; i++) {
       if(!strcmp(dir, mydirent[i].name) && (mydirent[i].mode & S_IFDIR)) {
  ...
  //Checks to see if file already exists
  for(i = 0; i < 100; i++) {
    if (strcmp(mydirent[i].name, path)==0) {
      return -EEXIST;
    }
  }
  ...
  for(i = 0; i < 100;  i++) { //Loops through all dirents till an empty dirent is found
    if (mydirent[i].valid == 0) {
      ...
      mydirent[i].valid = 1;
      mydirent[i].first_block = getnewfatent();// allocate a fatent
      
      if( mydirent[i].first_block == -1) {
         fprintf( stderr, "Error: no space on disk\n");
         return -1; // todo return the error code for running out of space
      }
      mydirent[i].size = 0; // though a fatent is allocated, size is zero

      //Fill in getuid, getgid mode and path
      mydirent[i].user = getuid();
      mydirent[i].group = getgid();
      mydirent[i].mode = mode;
      strcpy(mydirent[i].name, path);
      clock_gettime(CLOCK_REALTIME, &mydirent[i].access_time);
      clock_gettime(CLOCK_REALTIME, &mydirent[i].modify_time);
      clock_gettime(CLOCK_REALTIME, &mydirent[i].create_time);
      write_dirent(i);
                    ...
                    struct fuse_file_info *fi)
{
  ...
  for (int i = 0; i < 100; i++) {
    if (strcmp(mydirent[i].name, path) == 0 && mydirent[i].valid == 1) {
      fprintf( stderr, "for file %s size is %d, and its fats are: \n", mydirent[i].name, mydirent[i].size);
      printfats( mydirent[i].first_block);
      int readbytes = readfromdisk( mydirent[i].first_block, buf, size, offset, mydirent[i].size);
...
static int vfs_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
  ...
  for (int i = 0; i < 100; i++) {
    if (strcmp(mydirent[i].name, path) == 0 && mydirent[i].valid == 1) {
      int newsize = writetodisk( mydirent[i].first_block, buf, size, offset, mydirent[i].size);
      if(newsize != -1) {
         mydirent[i].size = newsize;
...
static int vfs_delete(const char *path) {
  ...
  for (int i = 0; i < 100; i++) {
    if (strcmp(mydirent[i].name, path) == 0 && mydirent[i].valid == 1) {
      mydirent[i].valid = 0;
      mydirent[i].name[0] = '\0';
      removefats(mydirent[i].first_block);
      write_dirent(i);
      return 0;
    }
  }
...
static int vfs_chmod(const char *file, mode_t mode) {
  for (int i = 0; i < 100; i++) {
    if (strcmp(mydirent[i].name, file) == 0 && mydirent[i].valid == 1) {
      mydirent[i].mode = (mode & 0x0000ffff); 
      return 0;
    }
  }
...
static int vfs_truncate(const char *file, off_t offset)
{
  ...
  for (int i = 0; i < 100; i++) {
    if (strcmp(mydirent[i].name, file) == 0 && mydirent[i].valid == 1) {
      ...
      printfats( mydirent[i].first_block);
      int newsize = truncatefat(mydirent[i].first_block, offset, mydirent[i].size); 
      if(newsize == -1) {
...
*/
int addnewblocks( fenum initial, int newblocks) {
  if (newblocks == 0) 
    return 0;
  ...
  fenum *arr = (fenum *)malloc( newblocks * sizeof(fenum));
...
int writetochained( fenum initial, const char *buf, int buflen, int offset) {
  ...
  // we start by finding out the first fenum to which we have to write, 
...
int writetodisk(fenum initial, const char *buf, int buflen, int offset, int filelen) {
  ...
  int curfileblocks;
  if( filelen == 0) 
    curfileblocks = 1;//cause  block is allocated when the file is created 
  else
    curfileblocks = (filelen - 1)/BLOCKSIZE; 
  ...
  if(filelen == 0) {//filelen==0 is a special case, cause a block is allocated already
    if( buflen <= BLOCKSIZE) {// we can just write to the 1st block
      ...
      fenum *allocatedblocks = (fenum *) malloc( nblocksrequired * sizeof (fenum));
...
int truncatefat( fenum initial, int offset, int filelen) {
  ...
  if(block(offset) == block( filelen)) {
    ...
    // find the fenum in which offset exists
    int offsetblock = block(offset +1); 
    int i = initial;
    int counter = 0;
    while(!fattable[i].eof) {
    ...
    // now i is the fenum after which we have to free all 
    if(fattable[i].eof == 1) {
